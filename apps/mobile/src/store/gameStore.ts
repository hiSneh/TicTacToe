...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  setDifficulty: (difficulty) => {
    trackEvent('difficulty_changed', { difficulty, platform: 'mobile' });
    set(({ game }) => ({ game: createGame({ ...game.config, difficulty }) }));
  },
  setMode: (mode) =>
    {
      trackEvent('game_mode_selected', { mode, platform: 'mobile' });
      set(({ game }) => ({
  ...
  playMove: (index) => {
    ...
    if (afterPlayer !== game) {
      trackEvent('move_played', { mode: game.config.mode, player: game.currentPlayer, index, platform: 'mobile' });
    }
