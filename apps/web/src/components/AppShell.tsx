...
import { activateRemoteConfig } from '../services/firebase';
...
export const AppShell = () => {
  ...
  useEffect(() => {
    void activateRemoteConfig().catch(() => undefined);
  }, []);

  useEffect(() => {
    trackScreenView(location.pathname === '/' ? 'home' : location.pathname.slice(1));
  }, [location.pathname]);
